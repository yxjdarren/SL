...
class GenericDataset_ll(data.Dataset):
    ...
    def __init__(self, dir_name, dataset_name, split, task_iter_item = 0, subtask_class_num=25, \
        ...
        if self.dataset_name=='cifar100':
            ...
            my_list = [i for i in range(25*(task_iter_item), 25*(task_iter_item+1))]
            subtask_all_id[task_iter_item] = get_ordered_elements(my_list, subtask_class_num)
...
def get_ordered_elements(input_list, k):
    if k <= len(input_list):
        return input_list[:k]
    else:
        return None
